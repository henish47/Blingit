...
@section('content')
    ...
    <div class="bg-gray-100 min-h-screen font-sans flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div
            class="relative w-full max-w-5xl bg-white rounded-3xl shadow-2xl border border-gray-200 p-6 sm:p-10 z-10 mx-auto overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                @if(session('success') && isset($order))
                    ...
                    <div class="flex flex-col items-center text-center mb-8">
                        <div class="bg-green-100 p-4 rounded-full mb-5 ring-4 ring-green-50">
                            <div class="bg-green-200 p-3 rounded-full">
                                <svg class="w-12 h-12 text-green-600" fill="none" stroke="currentColor" stroke-width="2.5"
                                    viewBox="0 0 24 24">
                        ...
                        <h1 class="text-3xl md:text-4xl font-extrabold text-gray-900 mb-2 fade-in-up"
                            style="animation-delay: 0.2s;">Thank You for Your Order!</h1>
                        <p class="text-gray-600 text-base md:text-lg max-w-2xl fade-in-up" style="animation-delay: 0.4s;">
                            {{ session('success') }}</p>
                        <p class="mt-4 text-lg font-bold text-gray-500 fade-in-up" style="animation-delay: 0.6s;">Order ID:
                            <span class="text-green-600">#BLINGIT-{{ $order->id }}</span></p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
                        ...
                        <div class="lg:col-span-3 fade-in-up" style="animation-delay: 0.8s;">
                                    ...
                                    <div class="bg-gray-50 rounded-2xl p-4 shadow-sm border border-gray-200">
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center gap-4">
                                                <img src="{{ $item->product->image_url ?? 'https://placehold.co/64x64' }}"
                                                    class="w-16 h-16 rounded-xl border border-gray-200 object-cover"
                                                    alt="{{$item->name}}"
                                                    onerror="this.onerror=null;this.src='https://placehold.co/64x64/f0f0f0/999999?text=Img';">
                                                <div>
                            ...
                            <div
                                class="flex justify-between items-center text-xl font-extrabold mt-6 pt-6 border-t border-gray-200">
                        ...
                        <div class="lg:col-span-2 space-y-8 fade-in-up" style="animation-delay: 1s;">
                            <div>
                                ...
                                <div
                                    class="bg-gray-50 rounded-2xl p-5 text-gray-700 text-base shadow-sm border border-gray-200">
                                    <p class="font-semibold text-gray-800">{{ $order->name }}</p>
                                    <p>{{ $order->address }}</p>
                                    <p>{{ $order->city }}, {{ $order->state }} - {{ $order->pincode }}</p>
                                    <p>Phone: {{ $order->phone }}</p>
                                    @if(!empty($order->alternate_phone))
                                        <p>Alt Phone: {{ $order->alternate_phone }}</p>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div
                                    class="bg-gray-50 rounded-2xl p-5 flex items-center gap-4 text-base font-semibold text-gray-800 shadow-sm border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-12 border-t border-gray-200 pt-8">
                        <a href="{{ route('home') }}"
                            class="flex-1 bg-green-600 hover:bg-green-700 text-white text-center py-3 px-6 text-base font-bold rounded-xl transition shadow-lg hover:shadow-green-500/30 hover:-translate-y-1 transform duration-300 ease-in-out flex items-center justify-center gap-2">
                        ...
                        <a href="{{ route('orders') }}"
                            class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 text-center py-3 px-6 text-base font-bold rounded-xl transition shadow-lg hover:shadow-gray-400/30 hover:-translate-y-1 transform duration-300 ease-in-out flex items-center justify-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <a href="{{ route('orders.invoice', $order) }}"
                            class="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-center py-3 px-6 text-base font-bold rounded-xl transition shadow-lg hover:shadow-blue-500/30 hover:-translate-y-1 transform duration-300 ease-in-out flex items-center justify-center gap-2"
                            target="_blank">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="my-10 border-t border-b border-gray-200 py-8">
                        <div>
                            ...
                            <form id="review-form"
                                class="bg-gray-50 rounded-2xl p-6 space-y-4 shadow-sm border border-gray-200 max-w-2xl mx-auto review-card"
                                novalidate>
                                <div class="text-center">
                                    ...
                                    <p id="rating-description"
                                        class="text-gray-600 text-sm mt-2 h-6 transition-all duration-300"></p>
                                </div>

                                <div>
                                    <textarea id="review-textarea" rows="3" placeholder="Tell us more about your experience..."
                                        class="w-full rounded-lg border border-gray-300 p-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 transition"></textarea>
                                ...
                                <button type="submit"
                                    class="w-full bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-lg font-semibold shadow-sm transition-transform hover:-translate-y-0.5 text-base">
                                    <span id="submit-text">Submit Feedback</span>
                                    <div id="submit-spinner" class="hidden"><i class="fas fa-spinner fa-spin"></i> Submitting...
                                    </div>
                                </button>
                            </form>

                            <div id="review-success"
                                class="hidden mt-6 bg-green-500 border border-green-600 rounded-2xl p-6 text-center max-w-2xl mx-auto review-success text-white">
                ...
                @else
                    <div class="text-center">
                        ...
                        <p class="text-gray-600 mt-2">It seems you've landed on this page by mistake. Let's get you back to
                            shopping!</p>
                        <a href="{{ route('home') }}"
                            class="mt-6 inline-block px-6 py-3 text-lg font-bold rounded-xl bling-btn">
                            Go to Homepage
                        </a>
                    </div>
                @endif
